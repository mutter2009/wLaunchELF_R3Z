...
//#endif //TMANIP
		...
		if (event || post_event) {  //NB: We need to update two frame buffers per event
			...
			// Update filename scroll offset for long highlighted names
			if (browser_sel != last_sel) {
				scroll_offset = 0;
				scroll_timer = 0;
				last_sel = browser_sel;
			}
			scroll_timer++;
			if (scroll_timer > 20) {  // scroll step every ~0.3s
				scroll_timer = 0;
				scroll_offset++;
			}
			...
			for (i = 0; i < rows; i++)  //Repeat loop for each browser text row
			{
				...
				// Prepare display name with scrolling for long highlighted filenames
				if (mcTitle != NULL) {
					name_src = mcTitle;
				} else {
				...
				if (name_limit > 0) {
					int char_w = (mcTitle != NULL) ? 16 : 8;
					int max_chars = name_limit / char_w;
					if (max_chars < 3) max_chars = 3;
					int total_chars = sjis_char_count_in_width(name_src, 65535);

					if (total_chars > max_chars) {
						if (top + i == browser_sel) {
							// Scroll long filename when highlighted
							int scroll_range = total_chars - max_chars + 2;
							int start_char = scroll_offset % (scroll_range + max_chars);
							if (start_char > total_chars - max_chars)
								start_char = 0;
							int start_bytes = sjis_skip_chars(name_src, start_char);
							int len = sjis_skip_chars(name_src + start_bytes, max_chars);
							memcpy(display_name, name_src + start_bytes, len);
							display_name[len] = '\0';
						} else {
							// Static truncation with '~' for non-highlighted items
							int len = sjis_skip_chars(name_src, max_chars - 1);
							memcpy(display_name, name_src, len);
							display_name[len] = '~';
