...
void drawChar2(int n, int x, int y, u64 colour)
{
	...
	// ELISA100.FNT glyphs are 8x8; upscale to 12x16 (height 16 like CJK, width 12
	// so each Japanese character fits one CN cell and long titles don't overlap details).
	for (i = 0; i < 8; i++) {
		b = elisaFnt[n + i];
		for (j = 0; j < 8; j++) {
			if (b & 0x80) {
			...
			switch (code) {
				...
				default:
					...
					if (n >= 0 && n <= 55008) {
						...
						x += 12;  // narrow Japanese glyph (12px wide)
					} else {
							if (draw)
								drawChar('_', x, y, colour);
							x += 8;
